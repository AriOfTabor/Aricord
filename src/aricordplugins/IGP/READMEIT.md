# IGP - Plugin di Crittografia PGP per Aricord

IGP (Aricord GPG/Pgp) è un potente plugin di crittografia per Aricord che permette la messaggistica cifrata end-to-end utilizzando la crittografia PGP (Pretty Good Privacy).
...
1. Posiziona la cartella del plugin nella directory `aricordplugins` di Aricord:
   ```
   Aricord/src/aricordplugins/IGP/
   ```

2. Ricostruisci Aricord per includere il plugin

3. Abilita il plugin nelle impostazioni di Aricord
...
1. Vai alle impostazioni di Aricord
...
IGP è stato sviluppato specificamente per la mod di Discord Aricord, estendendo l'architettura dei plugin Vencord. Il plugin sfrutta la libreria OpenPGP.js per fornire solide capacità di crittografia direttamente all'interno di Discord.
...
- **Archiviazione**: Usa l'API DataStore di Aricord per la gestione delle chiavi
- **Comandi**: Integrato con il sistema di comandi di Aricord
