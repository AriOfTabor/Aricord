...
import { checkForUpdates, isNewer, maybePromptToUpdate, update as updateAricord } from "@utils/updater";
...
const TELEGRAM_URL = "https://t.me/Aricord";
const REINSTALL_URL = "https://github.com/AriOfTabor/Aricord";
...
interface CrashReport {
    ...
    suspectedPluginCategory: "Equicord" | "Aricord" | "Vencord" | "User plugin" | "Unknown";
...
const settings = definePluginSettings({
    ...
    showSupportPopup: {
        type: OptionType.BOOLEAN,
        description: "Show the Aricord support popup after a crash.",
        default: true
    },
    detectBlankScreen: {
        type: OptionType.BOOLEAN,
    ...
    promptForUpdates: {
        type: OptionType.BOOLEAN,
        description: "Check for an Aricord update after the first crash in this session.",
        default: true
    },
    logCrashesToDisk: {
        type: OptionType.BOOLEAN,
...
function createReport(errorState: CrashErrorState): CrashReport {
    ...
    return {
        ...
        suspectedPluginCategory: folder.startsWith("src/equicordplugins/") ? "Equicord"
            : folder.startsWith("src/aricordplugins/") ? "Aricord"
...
async function checkAndUpdateAricord() {
    if (IS_WEB || IS_UPDATER_DISABLED) {
        showNotification({
            color: "#f23f43",
            title: "Aricord updater is not available.",
    ...
    try {
        ...
        if (!outdated) {
            showNotification({
                title: "Aricord is already up to date.",
        ...
        if (isNewer) {
            showNotification({
                color: "#f23f43",
                title: "Aricord cannot update automatically.",
        ...
        if (!await updateAricord()) return;

        Alerts.show({
            title: "Aricord updated.",
        ...
        logger.error("Failed to update Aricord from the crash popup.", err);
        showNotification({
            color: "#f23f43",
            title: "Aricord update failed.",
    ...
    recoveryTimer = setTimeout(() => {
        ...
        try {
            if (settings.store.promptForUpdates && !hasPromptedForUpdate) {
                ...
                maybePromptToUpdate("Aricord just caught a crash. If an update is available, it may fix the problem. Do you want to update now?", true);
            }
        } catch (err) {
        ...
        if (shouldNotify && settings.store.showRecoveryToast && !settings.store.showSupportPopup) {
            try {
                showNotification({
                    ...
                    title: latestCrash.report.recovered ? "Aricord recovered from the crash." : "Aricord recorded a crash.",
...
function CrashSupportModal({ modalProps, report }: CrashSupportModalProps) {
    ...
    const recoveredText = report.recovered
        ? "Aricord recovered the screen, but the crash can happen again if the install or a plugin is broken."
        : "Aricord could not confirm a clean recovery. Restart or reinstall the client before continuing.";
    const runUpdate = async () => {
        setIsCheckingUpdate(true);
        await checkAndUpdateAricord();
            ...
            title={(
                <div className={cl("header")}>
                    ...
                    <BaseText tag="span" size="lg" weight="semibold" className={cl("title")}>
                        Aricord caught a crash
                    </BaseText>
                </div>
            )}
            subtitle="Try reinstalling Aricord and check the Telegram group if the problem keeps happening."
        >
                    ...
                    <div className={cl("actions")}>
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Reinstall Aricord</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
                        ...
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Update Aricord</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
...
function CrashSupportFallback() {
    return (
        <div className={cl("fallback")} role="alertdialog" aria-modal="true" aria-label="Aricord crash recovery">
            <BaseText size="lg" weight="semibold">Aricord caught a crash</BaseText>
...
export default definePlugin({
    ...
    description: "Adds Aricord crash recovery, support guidance, and a copyable crash report.",
