...
import aricordIcon from "file://../../../browser/Aricord.png?base64";
...
const ARICORD_ICON = nativeImage.createFromDataURL(`data:image/png;base64,${aricordIcon}`);
...
export async function openInstance(
    ...
    try {
        const savedPartition = `persist:aricord-mi-${profileId}`;
        ...
        const win = new BrowserWindow({
            ...
            darkTheme: true,
            icon: mode === "detached" ? ARICORD_ICON : undefined,
            show: false,
            webPreferences: {
...
export async function clearSavedSession(
    ...
    try {
        const partition = `persist:aricord-mi-${profileId}`;
